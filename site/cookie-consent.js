...
if (typeof document !== "undefined") {
  const markup = `
    <div class="cookie-banner" data-cookie-banner hidden role="region" aria-label="Cookie notice">
      <div>
        <p class="eyebrow"><span class="dot dot-teal" aria-hidden="true"></span> Your choice</p>
        <h2>Small cookie, clear choice.</h2>
        <p>
          We use essential storage to remember this preference. Optional analytics stays off unless you choose it.
        </p>
      </div>
      <div class="cookie-actions">
        <button class="button button-paper" type="button" data-cookie-reject>Keep essentials only</button>
        <button class="button button-teal" type="button" data-cookie-accept>Allow optional analytics</button>
      </div>
    </div>
    <div class="cookie-overlay" data-cookie-dialog hidden>
      <section class="cookie-dialog" role="dialog" aria-modal="true" aria-labelledby="cookie-title">
        ...
        <h2 id="cookie-title">Choose what stays on.</h2>
        <p>
          Essential storage remembers your choice. Optional analytics is currently not loaded;
          this preference records whether it may be introduced later.
        </p>
        <label class="cookie-option">
          <span>
            <strong>Essential storage</strong>
            <small>Always active so your choice can be remembered.</small>
          </span>
        ...
        <label class="cookie-option">
          <span>
            ...
            <small>Off by default. Choose only if you want anonymous usage measurement.</small>
          </span>
  ...
  const setDialog = (open) => {
    dialog.hidden = !open;
    if (open) analytics.focus();
  };
  const save = (allowAnalytics) => {
    writeConsent(storage, allowAnalytics);
    banner.hidden = true;
    setDialog(false);
  };
  ...
  document
    ...
    ?.addEventListener("click", () => save(analytics.checked));
}
