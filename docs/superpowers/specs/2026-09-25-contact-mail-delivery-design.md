...
Replace the contact page's `mailto:` handoff with a server-side submission flow so visitors can submit an enquiry without being redirected to Outlook or another local email application. Enquiries are sent from `merchendices@gmail.com` using Gmail SMTP and delivered to the same mailbox.
...
3. The API sends a plain-text message with `from: merchendices@gmail.com`, `to: merchendices@gmail.com`, and `replyTo` set to the creator's submitted email.
